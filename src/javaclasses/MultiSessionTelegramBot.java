...
public class MultiSessionTelegramBot extends TelegramLongPollingBot {
    ...
    public Long getCurrentChatId() {
        if (updateEvent.get().hasMessage()) {
            return Long.valueOf(updateEvent.get().getMessage().getFrom().getId());
        }

        if (updateEvent.get().hasCallbackQuery()) {
            return Long.valueOf(updateEvent.get().getCallbackQuery().getFrom().getId());
        }
    ...
    public String getMessageText() {
        if (updateEvent.get().hasMessage()) {
            return updateEvent.get().getMessage().getText();
        } else return "";
    }
    ...
    public String getCallbackQueryButtonKey() {
        if (updateEvent.get().hasCallbackQuery()) {
            return updateEvent.get().getCallbackQuery().getData();
        } else return "";
    }
    ...
    public Message sendUrlButtonsMessage(String text, String... buttons) {
        SendMessage command = createApiSendMessageCommand(text);
        if (buttons.length > 0)
            attachUrlButtons(command, List.of(buttons));
    ...
    public void sendUrlButtonsMessage(String text, List<String> buttons) {
        SendMessage command = createApiSendMessageCommand(text);
        if (buttons != null && !buttons.isEmpty())
            attachUrlButtons(command, buttons);
    ...
    private void attachUrlButtons(SendMessage message, List<String> buttons) {
        InlineKeyboardMarkup markup = new InlineKeyboardMarkup();
        List<List<InlineKeyboardButton>> keyboard = new ArrayList<>();
    ...
    public static String loadRequests(String name) {
        try {
            var is = ClassLoader.getSystemResourceAsStream("requests/" + name + ".txt");
